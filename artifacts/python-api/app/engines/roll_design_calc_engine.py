...
def estimate_roll_od(
    ...
    # Geometry-derived baseline using section envelope + bend depth contribution.
    # This avoids fixed staircase OD jumps and tracks profile severity per station set.
    width = max(width, 1.0)
    height = max(height, 1.0)
    shaft_dia = max(shaft_dia, 0.0)

    envelope_diag = (width**2 + height**2) ** 0.5
    contour_severity = height * (1.0 + min(bend_count, 12) / 12.0)
    od = max(70.0, envelope_diag * 0.42 + contour_severity * 0.38 + thickness * 28.0)

    if station_count > 0:
        od += min(18.0, station_count * 0.8)

    if material in {"SS", "HR", "TI"}:
        od *= 1.07
    elif material in {"AL"}:
        od *= 0.97

    if section_type in {"lipped_channel", "door_frame"}:
        od *= 1.04
    elif section_type in {"complex_section", "complex_profile", "shutter_profile", "shutter_slat", "hat_section"}:
        od *= 1.08

    # Keep OD above shaft strength/packaging floor.
    if shaft_dia > 0:
        od = max(od, shaft_dia + 24.0)

    # Manufacturing rounding: nearest 1 mm for deterministic machining docs.
    return round(od, 3)
...
def estimate_spacer_logic(
    ...
    bend_count = max(bend_count, 0)

    envelope_diag = (width**2 + height**2) ** 0.5
    contour_relief = max(
        5.0,
        (0.20 * thickness * max(bend_count, 1)),
        (0.06 * envelope_diag),
    )
    section_complexity = 1.0
    if section_type in {"lipped_channel", "door_frame", "hat_section"}:
        section_complexity = 1.08
    elif section_type in {"complex_section", "complex_profile", "shutter_profile", "shutter_slat"}:
        section_complexity = 1.14

    working_face = max(26.0, (width + (2.0 * contour_relief)) * section_complexity)

    # Spacer stack from face width, shaft packaging and station loading.
    spacer_pack_factor = 0.14 + min(0.05, max(0, station_count) * 0.002)
    spacer_total = max(12.0, working_face * spacer_pack_factor, 0.22 * shaft_dia)
    ...
    return {
        ...
        "notes": [
            f"Section envelope considered: {round(width,3)} x {round(height,3)} mm",
            f"Thickness/Bend complexity considered: t={round(thickness,3)} mm, bends={bend_count}",
            f"Shaft dia considered {shaft_dia} mm",
            f"Station count considered {station_count}",
            f"Section type considered: {section_type}",
        ],
    }
