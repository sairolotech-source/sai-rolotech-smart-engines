import{b as O,j as t,aa as fe,l as Z,ay as $e,k as L,bt as be,t as ue,aj as ye,O as ge,q as we}from"./vendor-react-e4GBjkQt.js";import{J as Re}from"./vendor-pdf-6YqzeYCE.js";import{u as Se}from"./index-BvBPmCMg.js";import"./vendor-radix-mt19Efek.js";import"./vendor-misc-BxkwoVF7.js";import"./vendor-konva-Ca8PhNs_.js";import"./vendor-ui-utils-BmjxSsjX.js";import"./vendor-charts-BQqGqyK8.js";import"./vendor-query-ar9FQ0WI.js";import"./vendor-state-vUZwO_EA.js";function Y(l,n,c,s,r=!1){if(!l||l.length===0)return"";const p=[];let m=!0;return l.forEach(i=>{if(i.type==="line"){const W=c+i.startX*n,j=s+(r?-i.startY:i.startY)*n,b=c+i.endX*n,N=s+(r?-i.endY:i.endY)*n;m&&(p.push(`M ${W.toFixed(2)} ${j.toFixed(2)}`),m=!1),p.push(`L ${b.toFixed(2)} ${N.toFixed(2)}`)}else if(i.type==="arc"&&i.centerX!==void 0&&i.centerY!==void 0&&i.radius!==void 0){const W=c+i.centerX*n,j=s+(r?-i.centerY:i.centerY)*n,b=i.radius*n,N=(i.startAngle||0)*(Math.PI/180),y=(i.endAngle||360)*(Math.PI/180),z=W+b*Math.cos(N),e=j+(r?b*Math.sin(N):-b*Math.sin(N)),a=W+b*Math.cos(y),h=j+(r?b*Math.sin(y):-b*Math.sin(y));let f=y-N;f<0&&(f+=Math.PI*2);const o=f>Math.PI?1:0,d=r?1:0;m?(p.push(`M ${z.toFixed(2)} ${e.toFixed(2)}`),m=!1):p.push(`L ${z.toFixed(2)} ${e.toFixed(2)}`),p.push(`A ${b.toFixed(2)} ${b.toFixed(2)} 0 ${o} ${d} ${a.toFixed(2)} ${h.toFixed(2)}`)}}),p.join(" ")}function X(l){let n=1/0,c=-1/0,s=1/0,r=-1/0;return l.forEach(p=>{ve(p).forEach(i=>{n=Math.min(n,i.x),c=Math.max(c,i.x),s=Math.min(s,i.y),r=Math.max(r,i.y)})}),isFinite(n)?{minX:n,maxX:c,minY:s,maxY:r,w:c-n,h:r-s}:{minX:-50,maxX:50,minY:-10,maxY:10,w:100,h:20}}function ve(l){const n=[];if(l.type==="line")n.push({x:l.startX,y:l.startY},{x:l.endX,y:l.endY});else if(l.type==="arc"&&l.centerX!==void 0&&l.centerY!==void 0&&l.radius!==void 0){const c=(l.startAngle||0)*Math.PI/180;let r=(l.endAngle||360)*Math.PI/180-c;r<=0&&(r+=Math.PI*2);for(let p=0;p<=12;p++){const m=c+r*p/12;n.push({x:l.centerX+l.radius*Math.cos(m),y:l.centerY+l.radius*Math.sin(m)})}}return n}function $(l,n=2){return isFinite(l)?l.toFixed(n):"—"}function J(l,n,c,s,r,p,m){const e=148.33333333333334,a=60+40/2,h=X(c),f=s?.rollProfile?.upperRoll||[],o=s?.rollProfile?.lowerRoll||[],d=X(f.length>0?f:c),R=X(o.length>0?o:c),D=720/(h.w+40)*.8,T=e/(h.h+20)*.75,S=Math.min(D,T,3),x=720/(d.w+40)*.8,u=e/(d.h+20)*.75,g=Math.min(x,u,3),C=720/(R.w+40)*.8,P=e/(R.h+20)*.75,F=Math.min(C,P,3),w=800/2,v=a+e*.5,E=a+e*1.5,I=a+e*2.5,K=w-(d.minX+d.maxX)/2*g,Q=v-(d.minY+d.maxY)/2*g,A=Y(f.length>0?f:c,g,K,Q,!1),ee=w-(h.minX+h.maxX)/2*S,te=E-(h.minY+h.maxY)/2*S,_=Y(c,S,ee,te,!1),le=w-(R.minX+R.maxX)/2*F,se=I-(R.minY+R.maxY)/2*F,U=Y(o.length>0?o:c,F,le,se,!0),H=l+1,G=s?.upperRollOD||s?.rollProfile?.rollDiameter||0,ae=s?.upperRollID||s?.rollProfile?.shaftDiameter||0,k=s?.upperRollWidth||s?.rollProfile?.rollWidth||0,B=s?.lowerRollOD||s?.rollProfile?.rollDiameter||0,ne=s?.lowerRollID||s?.rollProfile?.shaftDiameter||0,M=s?.lowerRollWidth||s?.rollProfile?.rollWidth||0,oe=s?.rollGap??0,ie=s?.mfgSpec?.rollMaterial||"EN31 / D3",re=s?.mfgSpec?.rollHardness||"58–62 HRC",ce=s?.mfgSpec?.surfaceTreatment||"Hard Chrome / Nitride",de=s?.mfgSpec?.toleranceOD||"±0.02 mm",xe=s?.mfgSpec?.toleranceFace||"±0.01 mm",he=s?.behavior?.phase||"MAIN",pe=s?.behavior?.upperRollAction||"Upper forming roll",me=s?.behavior?.lowerRollAction||"Lower forming roll";return`<?xml version="1.0" encoding="UTF-8"?>
...
<svg xmlns="http://www.w3.org/2000/svg" width="800" height="600" viewBox="0 0 800 600">
  ...
  <text x="20" y="48" class="subtitle" fill="#93c5fd">Project: ${r}  |  Material: ${p}  |  Thickness: ${$(m)} mm</text>
  ...
  <text x="655" y="${a+50}" class="label">Width: <tspan class="value">${$(k)} mm</tspan></text>
  ...
  ${k>0?`
  <line x1="${w-k*g*.5}" y1="${v+e*.35}" x2="${w+k*g*.5}" y2="${v+e*.35}" class="dim-line"/>
  <line x1="${w-k*g*.5}" y1="${v+e*.28}" x2="${w-k*g*.5}" y2="${v+e*.4}" class="dim-line"/>
  <line x1="${w+k*g*.5}" y1="${v+e*.28}" x2="${w+k*g*.5}" y2="${v+e*.4}" class="dim-line"/>
  <text x="${w}" y="${v+e*.35+10}" text-anchor="middle" class="dim">Ø${$(G)} / w:${$(k)}</text>`:""}
...
</svg>`}function q({index:l,label:n,selected:c,onClick:s,hasRollData:r}){return t.jsxs("button",{onClick:s,className:`w-full text-left flex items-center gap-2 px-2 py-1.5 rounded-lg border transition-all ${c?"bg-blue-500/20 border-blue-500/50 text-blue-200":"bg-zinc-900/40 border-zinc-800/40 text-zinc-400 hover:border-zinc-700 hover:text-zinc-200"}`,children:[t.jsxs("span",{className:`text-[10px] font-black w-8 shrink-0 ${c?"text-blue-400":"text-zinc-600"}`,children:["ST",String(l+1).padStart(2,"0")]}),t.jsx("span",{className:"text-[10px] truncate",children:n}),t.jsx("span",{className:`ml-auto shrink-0 ${r?"text-green-500":"text-zinc-700"}`,children:r?t.jsx(we,{className:"w-3 h-3"}):t.jsx(L,{className:"w-3 h-3"})})]})}function Ie(){const{stations:l,rollTooling:n,geometry:c,materialThickness:s}=Se(),[r,p]=O.useState(0),[m,i]=O.useState("C-CHANNEL LINE-01"),[W,j]=O.useState(!1),[b,N]=O.useState(null),y=O.useCallback(o=>{const d=l[o],R=n[o]||null,D=d?.label||`Station ${o+1}`,T=d?.segments||[];return J(o,D,T,R,m,"CRCA",s||1.5)},[l,n,c,s,m]),z=O.useMemo(()=>l.length===0?J(0,"Demo Station 1",[{type:"line",startX:-80,startY:0,endX:-50,endY:0},{type:"arc",startX:-50,startY:0,endX:-40,endY:10,centerX:-50,centerY:10,radius:10,startAngle:270,endAngle:360},{type:"line",startX:-40,startY:10,endX:-40,endY:40},{type:"line",startX:-40,startY:40,endX:40,endY:40},{type:"line",startX:40,startY:40,endX:40,endY:10},{type:"arc",startX:40,startY:10,endX:50,endY:0,centerX:50,centerY:10,radius:10,startAngle:180,endAngle:270},{type:"line",startX:50,startY:0,endX:80,endY:0}],null,m,"CRCA",1.5):y(Math.min(r,l.length-1)),[y,r,l.length,m]),e=O.useCallback(async o=>{N(o);try{const d=y(o),D=(l[o]?.label||`Station_${o+1}`).replace(/\s+/g,"_").toUpperCase(),T=`ST${String(o+1).padStart(2,"0")}_${D}.svg`,S=new Blob([d],{type:"image/svg+xml"}),x=document.createElement("a");x.href=URL.createObjectURL(S),x.download=T,x.click(),URL.revokeObjectURL(x.href)}finally{N(null)}},[y,l]),a=O.useCallback(async()=>{j(!0);try{const o=new Re,d=o.folder("Roll_Data_Files"),R=Math.max(l.length,1);for(let x=0;x<R;x++){const u=y(x),C=(l[x]?.label||`Station_${x+1}`).replace(/\s+/g,"_").toUpperCase(),P=`ST${String(x+1).padStart(2,"0")}_${C}.svg`;d.file(P,u)}const D=["Station,Label,Upper OD,Upper ID,Upper Width,Lower OD,Lower ID,Lower Width,Roll Gap,Phase,Material"];for(let x=0;x<l.length;x++){const u=n[x],g=l[x];D.push([x+1,g?.label||`ST-${x+1}`,u?.upperRollOD||"",u?.upperRollID||"",u?.upperRollWidth||"",u?.lowerRollOD||"",u?.lowerRollID||"",u?.lowerRollWidth||"",u?.rollGap||"",u?.behavior?.phase||"MAIN",u?.mfgSpec?.rollMaterial||"EN31"].join(","))}d.file("Roll_Data_Index.csv",D.join(`
`));const T=await o.generateAsync({type:"blob"}),S=document.createElement("a");S.href=URL.createObjectURL(T),S.download=`${m.replace(/\s+/g,"_")}_Roll_Data_Files.zip`,S.click(),URL.revokeObjectURL(S.href)}finally{j(!1)}},[y,l,n,m]),h=Math.max(l.length,1),f=Math.min(r,h-1);return t.jsxs("div",{className:"flex flex-col h-full bg-zinc-950 overflow-hidden",children:[t.jsxs("div",{className:"flex items-center gap-2 px-3 py-1.5 border-b border-zinc-800/60 bg-zinc-900/50 shrink-0",children:[t.jsx(fe,{className:"w-4 h-4 text-blue-400 shrink-0"}),t.jsx("span",{className:"text-sm font-bold text-zinc-200",children:"Roll Data File Generator"}),t.jsx("span",{className:"text-[9px] text-zinc-500 hidden sm:block",children:"Har station ke liye alag SVG — UP roll / Strip profile / DOWN roll"}),t.jsxs("div",{className:"ml-auto flex items-center gap-2",children:[t.jsx("input",{value:m,onChange:o=>i(o.target.value),placeholder:"Project name...",className:"text-[10px] bg-zinc-800 border border-zinc-700 text-zinc-200 rounded px-2 py-0.5 w-40"}),t.jsxs("button",{onClick:a,disabled:W,className:"flex items-center gap-1.5 px-3 py-1 rounded border bg-emerald-500/15 border-emerald-500/40 text-emerald-300 hover:bg-emerald-500/25 text-[10px] font-bold transition-all disabled:opacity-50",children:[W?t.jsx(Z,{className:"w-3 h-3 animate-spin"}):t.jsx($e,{className:"w-3 h-3"}),"Download ZIP (All ",h," Files)"]})]})]}),l.length===0&&t.jsxs("div",{className:"flex items-center gap-2 px-3 py-2 bg-amber-500/10 border-b border-amber-500/20 text-[9px] text-amber-400",children:[t.jsx(L,{className:"w-3 h-3 shrink-0"}),"Demo mode — pehle Setup → Flower Pattern → Roll Tooling generate karo. Tabhi real roll data files export honge."]}),l.length>0&&n.length===0&&t.jsxs("div",{className:"flex items-center gap-2 px-3 py-2 bg-amber-500/10 border-b border-amber-500/20 text-[9px] text-amber-400",children:[t.jsx(L,{className:"w-3 h-3 shrink-0"}),'Stations mil gaye — lekin Roll Tooling abhi generate nahi hua. Roll Tooling tab mein jaake "Generate" karo phir vapas ao.']}),t.jsxs("div",{className:"flex flex-1 overflow-hidden",children:[t.jsxs("div",{className:"w-44 shrink-0 border-r border-zinc-800/60 bg-zinc-900/30 flex flex-col overflow-hidden",children:[t.jsx("div",{className:"px-2 py-1.5 border-b border-zinc-800/40",children:t.jsxs("span",{className:"text-[9px] text-zinc-500 uppercase tracking-widest",children:["Stations (",h,")"]})}),t.jsx("div",{className:"flex-1 overflow-y-auto p-1.5 space-y-0.5",children:l.length>0?l.map((o,d)=>t.jsx(q,{index:d,label:o.label||`Station ${d+1}`,selected:f===d,onClick:()=>p(d),hasRollData:!!n[d]},d)):t.jsx(q,{index:0,label:"Demo Station 1",selected:!0,onClick:()=>{},hasRollData:!1})}),t.jsxs("div",{className:"p-2 border-t border-zinc-800/40 flex gap-1",children:[t.jsx("button",{onClick:()=>p(o=>Math.max(0,o-1)),disabled:f===0,className:"flex-1 flex items-center justify-center py-1 rounded border border-zinc-700/40 text-zinc-500 hover:text-zinc-200 disabled:opacity-30 transition-all",children:t.jsx(be,{className:"w-3.5 h-3.5"})}),t.jsxs("span",{className:"text-[9px] text-zinc-600 flex items-center justify-center w-12",children:[f+1,"/",h]}),t.jsx("button",{onClick:()=>p(o=>Math.min(h-1,o+1)),disabled:f===h-1,className:"flex-1 flex items-center justify-center py-1 rounded border border-zinc-700/40 text-zinc-500 hover:text-zinc-200 disabled:opacity-30 transition-all",children:t.jsx(ue,{className:"w-3.5 h-3.5"})})]}),t.jsxs("button",{onClick:()=>e(f),disabled:b!==null,className:"mx-2 mb-2 flex items-center justify-center gap-1.5 py-1.5 rounded border bg-blue-500/15 border-blue-500/40 text-blue-300 hover:bg-blue-500/25 text-[10px] font-bold transition-all disabled:opacity-50",children:[b===f?t.jsx(Z,{className:"w-3 h-3 animate-spin"}):t.jsx(ye,{className:"w-3 h-3"}),"Download ST-",String(f+1).padStart(2,"0")]})]}),t.jsxs("div",{className:"flex-1 overflow-hidden flex flex-col",children:[t.jsxs("div",{className:"flex items-center gap-2 px-3 py-1 border-b border-zinc-800/40 bg-zinc-900/20 shrink-0",children:[t.jsx(ge,{className:"w-3 h-3 text-zinc-500"}),t.jsxs("span",{className:"text-[9px] text-zinc-500",children:["Preview: ST-",String(f+1).padStart(2,"0")," — ",l[f]?.label||"Demo Station"]}),t.jsxs("div",{className:"ml-auto flex gap-2 text-[8px] text-zinc-600",children:[t.jsxs("span",{className:"flex items-center gap-1",children:[t.jsx("span",{className:"w-3 h-0.5 bg-blue-500 inline-block"})," UP Roll"]}),t.jsxs("span",{className:"flex items-center gap-1",children:[t.jsx("span",{className:"w-3 h-0.5 bg-emerald-500 inline-block"})," Strip"]}),t.jsxs("span",{className:"flex items-center gap-1",children:[t.jsx("span",{className:"w-3 h-0.5 bg-red-500 inline-block"})," DN Roll"]})]})]}),t.jsx("div",{className:"flex-1 overflow-auto bg-zinc-800/30 p-4 flex items-center justify-center",children:t.jsx("div",{className:"bg-white rounded shadow-2xl max-w-full",style:{maxHeight:"calc(100vh - 160px)"},dangerouslySetInnerHTML:{__html:z}})})]})]})]})}export{Ie as RollDataFileGenerator};
