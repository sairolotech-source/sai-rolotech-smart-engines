...
def _resample_numeric_schedule(values: List[float], target_count: int) -> List[float]:
    """Resample a numeric schedule to target_count while preserving endpoints."""
    if target_count <= 0 or not values:
        return []
    if len(values) == target_count:
        return [round(v, 1) for v in values]
    if len(values) == 1:
        return [round(values[0], 1)] * target_count
    ...
    for i in range(target_count):
        ...
        if hi == lo:
            ...
            frac = pos - lo
            val = values[lo] + (values[hi] - values[lo]) * frac
        out.append(round(val, 1))
    return out


def _extract_flower_angle_schedule(
    flower_result: Dict[str, Any],
    target_deg: float,
    forming_passes: int,
) -> List[float]:
    """
    Extract pass-wise target bend angles from advanced_flower pass_plan.
    Returns [] when pass_plan is missing or unusable.
    """
    pass_plan = flower_result.get("pass_plan", [])
    if not isinstance(pass_plan, list) or not pass_plan:
        return []

    forming_entries = [p for p in pass_plan if not bool(p.get("is_calibration", False))]
    if not forming_entries:
        forming_entries = pass_plan

    raw_angles: List[float] = []
    for entry in forming_entries:
        bends = entry.get("bend_angles_deg", [])
        vals: List[float] = []
        ...
        if vals:
            ...
            pct = _safe_float(entry.get("progression_pct"), 0.0)
            angle = target_deg * max(0.0, min(100.0, pct)) / 100.0

        raw_angles.append(round(max(0.0, min(target_deg, angle)), 1))

    return _resample_numeric_schedule(raw_angles, forming_passes)


def _extract_flower_stage_labels(
    flower_result: Dict[str, Any],
    forming_passes: int,
) -> List[str]:
    """Extract and align pass labels from advanced_flower pass_plan to forming passes."""
    pass_plan = flower_result.get("pass_plan", [])
    if not isinstance(pass_plan, list) or not pass_plan:
        return []
    ...
    labels = [str(p.get("label", "")).strip() for p in forming_entries]
    labels = [lbl for lbl in labels if lbl]
    if not labels or forming_passes <= 0:
        return []
    if len(labels) == forming_passes:
        return labels
    if len(labels) == 1:
        return [labels[0]] * forming_passes

    out: List[str] = []
    src_last = len(labels) - 1
    for i in range(forming_passes):
        if forming_passes == 1:
            src_idx = 0
        else:
            src_idx = round((i * src_last) / (forming_passes - 1))
        out.append(labels[src_idx])
    return out
...
def generate_roll_contour(
    ...
    pass_plan = flower_result.get("pass_plan", [])
    has_pass_plan = isinstance(pass_plan, list) and len(pass_plan) > 0

    angles = _extract_flower_angle_schedule(
        flower_result=flower_result,
        target_deg=angle_with_springback,
        forming_passes=forming_passes,
    )
    angle_schedule_source = "flower_pass_plan"
    if not angles:
        if has_pass_plan:
            reason = (
                "Flower pass_plan exists but no valid per-pass bend angles could be extracted. "
                "Refusing heuristic fallback for contour derivation."
            )
            logger.warning("[roll_contour] %s", reason)
            return {
                "status": "fail",
                ...
                "details": {
                    "forming_passes": forming_passes,
                    "target_overbend_deg": angle_with_springback,
                    "pass_plan_length": len(pass_plan),
                },
            }
        ...
        angle_schedule_source = "profile_heuristic_no_pass_plan"
    flower_stage_labels = _extract_flower_stage_labels(flower_result, forming_passes)
    ...
    for i, (angle, sw) in enumerate(zip(angles, strip_widths)):
        ...
        default_stage = _stage_label(i, forming_passes, has_lips, profile_type)
        flower_stage = flower_stage_labels[i] if i < len(flower_stage_labels) else ""
        stage_type = flower_stage if flower_stage else default_stage
        ...
        passes.append({
            ...
            "stage_type_default": default_stage,
            "flower_stage_label": flower_stage,
            "geometry_grade":     geom_grade_pass,
    ...
    return {
        ...
        "angle_schedule_source": angle_schedule_source,
        "flower_pass_plan_used": angle_schedule_source == "flower_pass_plan",
        "passes":            passes,
