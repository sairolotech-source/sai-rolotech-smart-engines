...
function AnimatedCounter({ target, suffix = "", delay = 0.5 }: { target: number; suffix?: string; delay?: number }) {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      const iv = setInterval(() => { c += step; if (c >= target) { setVal(target); clearInterval(iv); } else { setVal(Math.round(c * 10) / 10); } }, 40);
      return () => clearInterval(iv);
    }, delay * 1000);
    return () => clearTimeout(t);
  }, [target, delay]);
  return <span>{val.toFixed(suffix === "mm" ? 1 : 0)}{suffix}</span>;
}
...
function SceneSetup() {
  ...
  useEffect(() => { const t = setInterval(() => setStep(s => Math.min(s + 1, 4)), 2500); return () => clearInterval(t); }, []);
  const fields = [
        ...
        <div className="flex-1">
          <motion.div className="bg-zinc-900 border border-green-500/30 rounded-xl p-6" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 3.5 }}>
            ...
            <div className="space-y-2 text-sm font-mono">
              ...
              <div className="flex justify-between"><span className="text-zinc-400">Strip Width:</span><span className="text-orange-400"><AnimatedCounter target={247.3} suffix="mm" delay={4} /></span></div>
...
export function DemoVideoCenter() {
  ...
  useEffect(() => {
    if (!isPlaying) return;
    const dur = DEMO_SCENES[activeScene].duration;
