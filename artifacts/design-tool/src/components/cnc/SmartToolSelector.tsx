...
import { Cpu, ChevronDown, ChevronRight, Clock, Database } from "lucide-react";
...
function matchLibraryTool(tools: LibraryTool[], operation: string, materialType: string, insertCode: string): LibraryTool | null {
  ...
  const familyMatch = candidates.find(t =>
    t.isoDesignation && t.isoDesignation.startsWith(family + " ")
  );
  ...
  const gradeMatch = candidates.find(t =>
    t.gradeCode && preferredGrades.includes(t.gradeCode)
  );
...
export function SmartToolSelector() {
  const { rollTooling, materialType, rollDiameter } = useCncStore();
    ...
    setTimeout(() => {
      ...
      const buildRec = (operation: string, fallbackInsert: string, fallbackFamily: string, fallbackHolder: string, vc: number, feed: number, doc: number): ToolRecommendation => {
        ...
        if (libTool) {
          return {
            operation,
            insertCode: libTool.isoDesignation || libTool.name,
            insertFamily: (libTool.isoDesignation || libTool.name).split(" ")[0],
            holder: libTool.holderCode || fallbackHolder,
            vc, feed, doc,
            rpm: getRpmRange(vc, rollDia),
            reason: `From tool library: ${libTool.name} (${libTool.gradeCode || ""} ${libTool.coatingType || ""})`.trim(),
            fromLibrary: true,
          };
        }
        return {
          operation, insertCode: fallbackInsert, insertFamily: fallbackFamily, holder: fallbackHolder,
          vc, feed, doc, rpm: getRpmRange(vc, rollDia),
          reason: ins.reason, fromLibrary: false,
        };
      };
      ...
      const recs: ToolRecommendation[] = [
        ...
        buildRec("OD Finish Profile",
          materialType === "AL" ? "VCMT 110302-AK" : materialType === "SS" ? "VNMG 110302-PF" : "VNMG 110304-PF",
          "VNMG", "MVJNR 2020K-11", ins.vcFinish, ins.feedFinish, ins.docFinish),
        buildRec("Boring (ID Finish)",
          materialType === "AL" ? "CCMT 060202-AK" : "CCMT 060204-MF",
          "CCMT", `Boring bar Ø${Math.max(16, Math.round(rollDia * 0.25))} carbide`,
          ins.vcFinish * 0.7, ins.feedFinish * 0.8, 0.3),
        buildRec("Grooving",
          materialType === "AL" ? "MGMN 200-G (2mm)" : "MGMN 250-G (2.5mm)",
          "Grooving", "MGEHR 2020-3 blade holder",
          ins.vcRough * 0.5, materialType === "SS" ? 0.04 : 0.06, 0.5),
      ];
