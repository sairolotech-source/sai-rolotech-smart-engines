...
- **Python FastAPI Server:** Runs at `artifacts/python-api/` with a modular architecture of 45+ distinct engineering engines, providing endpoints for auto/manual mode calculations, DXF upload, semi-auto confirmation, PDF export, CAD export, simulation, BOM, process card, and debugging. Features an Accuracy Control System for confidence scoring and automated review mode determination (Auto, Semi Auto, Manual Review).
- **Phase C — Machine Configuration Store + Machine-Aware Validation (v2.3.0):** `app/utils/machine_config_store.py` — 4 built-in machines (SAI-LITE-12/SAI-STD-20/SAI-HD-30/SAI-CNC-24) with full master data model (machine_id, stand_count, shaft_diameter_mm, max_roll_od_mm, strip_width_min/max_mm, thickness_min/max_mm, max_line_speed_mpm, motor_power_kw, supported_materials, calibration_offsets, pass_limit_rules, tooling_constraints). `app/engines/machine_aware_validator.py` — `validate_job_on_machine()` cross-validates any profile+machine, returns feasibility/blocking/warnings/adjusted_params/process_constraints/tooling_recommendation; `select_best_machine()` auto-selects best fit. Station engine v2.1 — optional `machine_config` parameter clamps station count to machine stand_count, blocks unsupported materials. Process card engine updated — optional `machine_constraints` dict adds machine_id and full setup_constraints to card header (gap correction, angle correction, tension factor, line speed, shaft dia, bearing series, max OD). Tooling library — `get_best_match_for_machine()` filters by shaft dia + OD limits; `check_tooling_machine_compatibility()` verifies a specific tooling entry. 30 Phase C tests all pass.
- **Phase A — Top-50 Profile Matrix (v2.3.0):** `app/utils/top50_profile_matrix.py` — 50 commercial roll forming profiles across 15 section families (c_channel×6, lipped_channel×9, z_purlin×2, z_section×2, hat_section×2, sigma_section×3, omega_section×3, angle_section×3, trapezoid×4, shutter_slat×4, shutter_profile×2, door_frame×3, box_section×2, square_tube×2, rect_tube×3). Each profile: exact tooling_id (27 unique library entries referenced), machine-wise compatibility for all 4 machines, validation_status (46 validated/4 rejected), thickness 0.5–5.0mm, materials GI/CR/HR/MS/SS/AL/HSLA. Machine coverage: LITE=18 profiles, STD=33, HD=41, CNC=44. 4 rejected profiles are wide-format roofing sheets (strip width >800mm, requires specialized wide-coil mill not in current registry — technically correct rejection). Matrix functions: `get_profile_matrix()`, `matrix_summary()`, `get_profile_by_id()`.
- **COPRA-Class Audit (v2.2.0):** Full engineering audit completed April 2026. Report at `artifacts/python-api/COPRA_AUDIT_REPORT_v2_2.md`. 4 new engines implemented: (1) `bom_engine.py` — Bill of Materials (10 line item categories, rolls/shafts/bearings/spacers/keys/side rolls/entry guides/hardware/decoiler/runout); (2) `bend_allowance_engine.py` — DIN 6935 K-factor flat blank calculator (flat_blank_mm, coil_strip_width_mm, weight_kg_per_m, min radius warning); (3) `process_card_engine.py` — per-station process parameter card (angle/springback/gap/force/power/strain/defect risk/operator notes + rendered text table); (4) `material_database.py` — complete property DB for 10 materials (Fy/UTS/E/elongation/n-value/r-value/k-factor/density). 4 new API routes: GET /api/materials, POST /api/bend-allowance, POST /api/bom, POST /api/process-card. Test coverage expanded from 7 to 12 test files, 351 tests, 100% pass rate.
