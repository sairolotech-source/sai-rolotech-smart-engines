...
def generate_cam_prep(
    ...
    # Build per-station roll specs
    rolls: List[Dict[str, Any]] = []
    for i, p in enumerate(passes_data):
        depth = round(p.get("forming_depth_mm", 0), 2)
        roll_spec = {
            ...
            "od_mm":           roll_od,
            "bore_mm":         shaft_dia,
            "face_width_mm":   round(p["strip_width_mm"] + 10, 1),
    ...
    # Calibration roll
    cal_spec = {
        "roll_label":      f"Stand {n_stations} Cal. Upper + Lower",
        "station_no":      n_stations,
        "stage_type":      "calibration",
        "od_mm":           roll_od,
        "bore_mm":         shaft_dia,
        "face_width_mm":   roll_calc_result.get("spacer_recommendation", {}).get("suggested_working_face_mm", 50),
        "profile_depth_mm": round(roll_contour_result.get("calibration_pass", {}).get("forming_depth_mm", 5), 2),
        "hardness_hrc":    roll_props["hardness_hrc"],
        "roll_material":   roll_material,
    }
    rolls.append({
    ...
    solidcam_bridge = {
        ...
        "stock_od_mm":    roll_od + 6,
        "stock_bore_mm":  shaft_dia - 4,
        "stock_length_mm": rolls[0]["face_width_mm"] + 10 if rolls else 60,
