...
function detectSectionModel(geometry: { segments: Array<{ startX: number; startY: number; endX: number; endY: number }>; boundingBox: { minX: number; maxX: number; minY: number; maxY: number } }): "open" | "closed" {
  ...
  // If gap between start and end points < 2% of profile span â†’ closed
...
export function LeftPanel() {
  ...
  // Only applies to file uploads â€” manual profiles drawn via wizard do not require confirmation.
  ...
  // â”€â”€ Typed notification dispatcher (replaces setError("âœ… ...") anti-pattern) â”€
  ...
  // â”€â”€ Python API health probe â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Line-to-Sheet offset â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const applySheetOffset = React.useCallback(() => {
    ...
    setGeometry(newGeo);
    toast({ title: "Sheet Offset Applied", description: `Mode: ${sheetOffsetMode === "both" ? "Both Sides Â±" + (t / 2).toFixed(2) : sheetOffsetMode === "up" ? "Single Up +" + t : "Single Down -" + t} mm` });
  ...
  const handleApplyPostProcessor = useCallback(() => {
    if (selectedPP) {
      ...
      notify.success(`Post Processor: ${selectedPP.name} applied â€” saZ=${selectedPP.machineProfile.safeZ} | ${selectedPP.machineProfile.spindleDirection} | ${selectedPP.machineProfile.feedUnit}`);
    }
  ...
  // â”€â”€ Auto Shaft Diameter Calculation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Based on Roll OD ratio rule: shaft = 25â€“30% of roll OD (industry standard)
  ...
  // the effect â€” BUT if any parent re-render causes a new autoShaftValue to be
  ...
  // actually changes, breaking the render â†’ effect â†’ set â†’ render cycle.
  ...
  const handleGenerateGcode = useCallback(async () => {
    ...
    try {
      ...
      if (result.gcodeOutputs && result.gcodeOutputs.length > 0) {
        ...
        scoreTask("gcode", `G-Code â€” ${numStations} stations`, {
  ...
  const handleGenerateRollTooling = useCallback(async () => {
    ...
    if (!validation.valid) {
      const msg = validation.errors[0] ?? "Invalid inputs â€” check roll diameter and thickness.";
    ...
    try {
      ...
      const normalizedRollTooling = (result.rollTooling ?? []).map((rt: any) => {
        // Only short-circuit if rollProfile is complete â€” passLineY must be present and finite.
      ...
      if (result.rollTooling && result.rollTooling.length > 0) {
        ...
        scoreTask("tooling", `Roll Tooling â€” ${materialType} ${numStations} stations`, {
        ...
        if (withProfile < total) {
          toast({
            ...
            description: `${withProfile}/${total} stations have roll profiles â€” ${total - withProfile} need regeneration`,
            variant: "destructive",
          });
        } else {
          toast({ title: "Roll Tooling Generated", description: `${total} roll sets ready â€” all profiles complete` });
        }
      }
    } catch (err: unknown) {
  ...
  // â”€â”€â”€ Regenerate incomplete stations â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const handleRegenerateIncomplete = useCallback(async () => {
    ...
    toast({
      ...
      description: `Regenerating ${incomplete.length} station(s) â€” rerunning full roll tooling pipeline`,
    });
  ...
  return (
    ...
    <CenterLineConversionModal
      ...
      onConverted={(bendCount, devLength) => {
        toast({
          ...
          description: `${bendCount} bends detected Â· Developed length: ${devLength.toFixed(1)}mm. Now configure stations and generate Power Pattern.`,
        });
      }}
    />
    ...
    <div ref={leftPanelRef} className="w-72 flex flex-col overflow-y-auto flex-shrink-0"
            ...
            <div className="text-[10px] font-normal mt-0.5 truncate">
              {autoAI.enabled
                ? autoAI.status.step === "idle"
                  ? "Profile load karo â€” auto run hoga"
                  : autoAI.status.step === "done"
                  ? `Complete â€” Score: ${autoAI.status.manufacturabilityScore !== undefined ? autoAI.status.manufacturabilityScore + "/100" : "âœ“"}`
                  : autoAI.status.step === "error"
                  ? autoAI.status.error ?? "Error aaya"
                  : autoAI.status.message
                : "AI-powered full pipeline: Flower â†’ Tooling â†’ G-Code"
              }
            </div>
          </div>
      ...
      {/* â”€â”€ OPERATOR NEXT STEP PANEL â”€â”€ */}
      {(() => {
        ...
        const steps: Step[] = [
          ...
          },
          {
            ...
            icon: hasStations ? "âœ…" : "â‘¡",
            label: hasStations ? `Flower pattern: ${stations.length} stations` : "Run Flower Pattern",
            detail: hasStations ? "Power Pattern generated â€” pass schedule ready" : "Click 'Generate Power Pattern' in the Station section",
            color: hasStations ? "#22c55e" : hasGeometry ? "#f59e0b" : "#52525b",
          },
          {
            done: hasTooling && incompleteCount === 0,
            icon: hasTooling && incompleteCount === 0 ? "âœ…" : hasTooling ? "âš " : "â‘¢",
            label: hasTooling
            ...
            detail: hasTooling
              ? incompleteCount > 0 ? `${incompleteCount} station(s) missing profile â€” use âŸ³ Regenerate below` : "All roll profiles complete"
          ...
          },
          {
            ...
            detail: hasGcode ? "G-Code generated â€” run CNC safety check before export" : "Click 'Generate G-Code' in the G-Code section",
          ...
          return (
            <div className="mx-3 my-2 rounded-lg border border-emerald-800/50 bg-emerald-950/25 p-2.5">
              <div className="text-[9px] font-bold uppercase tracking-widest text-emerald-400 mb-1.5 flex items-center gap-1">
                <span>âœ…</span> Package Ready
              </div>
              <div className="text-[10px] text-zinc-300 leading-tight">
                Profile Â· Flower Â· Tooling Â· G-Code all complete.
      ...
      {/* â”€â”€ FILE UPLOAD QUICK ACCESS â”€â”€ */}
      <div className="p-3 border-b border-white/[0.05]">
        {fileName && geometry ? (
          /* File loaded â€” status + actions */
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/25">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[9px] text-emerald-600">
                  {(geometry.segments ?? []).length} seg Â· {(geometry.bendPoints ?? []).length} bends Â·{" "}
          ...
          /* No file â€” big drag-and-drop upload area */
          <label className="flex flex-col items-center justify-center gap-2 w-full py-5 border-2 border-dashed border-white/[0.10] rounded-xl cursor-pointer hover:border-blue-500/50 hover:bg-blue-500/[0.05] transition-all group">
            ...
            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/[0.04] border border-white/[0.06] text-[9px] text-zinc-500">
              .dxf supported &nbsp;Â·&nbsp; .dwg requires server converter
            </span>
            <input type="file" accept=".dxf,.dwg" className="hidden" onChange={handleFileUpload} />
          </label>
        )}
      ...
      {/* â”€â”€ AUTOCAD EDITOR QUICK LAUNCH â”€â”€ */}
      <div className="p-3 border-b border-white/[0.05]">
        <button
        ...
        >
          ...
          <div className="flex-1 text-left">
            ...
            <p className="text-[10px] text-zinc-500 group-hover:text-zinc-400 mt-0.5">
              LINE Â· CIRCLE Â· ARC Â· POLYLINE Â· OFFSET Â· TRIM
            </p>
          </div>
      ...
      <div className="p-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.045)" }}>
        ...
        {sections.project && (
          <div className="mt-3 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <div className="text-[10px] text-zinc-500 mt-0.5">
                  Mat. range: {matProps.minThickness} â€“ {matProps.maxThickness} mm
                </div>
                {!isThicknessValid && (
            ...
            {/* â”€â”€ Thickness Band (min / nominal / max) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
            <div className="bg-zinc-900/60 border border-zinc-700/50 rounded-lg p-2.5 space-y-1.5">
              <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-wide flex items-center gap-1.5">
                <span>ðŸ“</span> Thickness Band
                <span className="text-zinc-600 font-normal normal-case">â€” for roll gap + strip width conservatism</span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <input
                    ...
                    className={inputCls}
                    title="Minimum tolerance â€” used for strip width (tightest fit)"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className={inputCls}
                    title="Maximum tolerance â€” used for roll gap lower bound (conservative)"
                  />
                </div>
              </div>
              {thicknessBandMax < materialThickness && (
                <div className="text-[9px] text-red-400 flex items-center gap-1">
                  <AlertTriangle className="w-3 h-3 flex-shrink-0" /> Max thickness is below nominal â€” gap may be too tight
                </div>
              )}
              {thicknessBandMin > materialThickness && (
                <div className="text-[9px] text-red-400 flex items-center gap-1">
                  <AlertTriangle className="w-3 h-3 flex-shrink-0" /> Min thickness is above nominal â€” strip width calc incorrect
                </div>
              )}
              <div className="text-[9px] text-zinc-600">
                Band: Â±{(((thicknessBandMax - thicknessBandMin) / 2 / materialThickness) * 100).toFixed(1)}% Â· Roll gap uses max ({thicknessBandMax} mm) Â· Strip width uses min ({thicknessBandMin} mm)
              </div>
            </div>

            {/* â”€â”€ Profile Source Type â€” auto-detect + manual override â”€â”€â”€â”€â”€â”€â”€â”€ */}
            {(() => {
              ...
              const srcDesc: Record<string, string> = {
                centerline:   "Offset Â±t/2 both sides â†’ neutral axis",
                inner_face:   "Offset outward by full thickness",
                outer_face:   "Offset inward by full thickness",
                sheet_profile: "No offset â€” already full sheet",
              };
              ...
              return (
                <div className="bg-zinc-900/60 border border-zinc-700/50 rounded-lg p-2.5 space-y-2">
                  <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-wide flex items-center gap-1.5">
                    <span>ðŸ“</span> Profile Source Type
                    {profileSourceType && (
                  ...
                  {!profileSourceType && geometry && (
                    <div className="text-[9px] text-amber-400">âš  Profile source not detected â€” select manually above before generating flower pattern</div>
                  )}
                </div>
              );
            })()}
      ...
      <div className="p-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.045)" }}>
        <SectionHeader title="Material Intelligence" icon={<span className="text-sm">ðŸ§ª</span>} expanded={sections.material} onToggle={() => toggleSection("material")} />
        {sections.material && (
          <div className="mt-3 space-y-3">
                ...
                {[
                  ...
                  return (
                    <div key={key} className="flex justify-between text-[9px]" title={ci.method || ""}>
                      ...
                      <span className="text-amber-300 font-mono">{ci.low}â€“{ci.high} {ci.unit}</span>
                    </div>
                  );
                })}
              </div>
            ...
            {materialThickness >= 5.0 && (
              <div className="flex items-start gap-1.5 bg-orange-950 border border-orange-800 rounded p-2 text-[10px] text-orange-300">
                <AlertTriangle className="w-3 h-3 flex-shrink-0 mt-0.5" />
                <div>
                  <div className="font-bold">Heavy Gauge â‰¥ 5mm</div>
                  <div className="opacity-80">Use roll OD â‰¥ 200mm, EN31/D2 rolls, slow speed &lt; 15 m/min. Calibration stand mandatory.</div>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="p-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.045)" }}>
        <SectionHeader title="Profile Type" icon={<span className="text-sm">â—§</span>} expanded={sections.profileType ?? true} onToggle={() => toggleSection("profileType" as keyof typeof sections)} />
        <div className="mt-3 space-y-2">
          <div>
              ...
              return (
                <div className="rounded-lg border p-2.5 text-[10px] space-y-1.5 bg-orange-950 border-orange-800 text-orange-300">
                  ...
                  {geometry && detectedType ? (
                    ...
                    <div className="opacity-80">DXF file upload karein â€” profile type automatically detect hoga</div>
                  )}
                </div>
              );
            }
              ...
              <div className={`rounded-lg border p-2.5 text-[10px] space-y-1 ${
                ...
                {isPopSection && (
                  <div className="border-t border-amber-800 pt-1 font-semibold">
                    â–² Pop/Embossed: Bend angle 90â€“120Â° â€” springback Ã—1.15 applied
                  </div>
                )}
              </div>
            );
          ...
          <div className="space-y-1.5 pt-1">
            ...
            >
              <div className="font-bold">ðŸ­ Shutter Plant Preset</div>
              <div className="text-[10px] text-blue-500 mt-0.5">GI 2mm Â· C-Section Â· Ã˜150 roll Â· 40mm shaft Â· 14 stations Â· 30 m/min</div>
            </button>
            <button
            ...
            >
              <div className="font-bold">â–² Pop / Angle Preset</div>
              <div className="text-[10px] text-amber-500 mt-0.5">MS 2mm Â· Pop/Embossed Â· Ã˜160 roll Â· 40mm shaft Â· 18 stations Â· springback Ã—1.15</div>
            </button>
          </div>
        </div>
      </div>
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      {(geometry?.segments ?? []).length > 0
                        ? <p className="text-[10px] text-green-500">Parsed â€” {geometry.segments.length} segments, {(geometry.bendPoints ?? []).length} bends detected</p>
                        : <p className="text-[10px] text-amber-400">âš  File parsed but no segments found â€” check DXF layer/entity type</p>
                      }
                    </div>
                  </div>
                  <button
                  ...
                  {(geometry.bendPoints ?? []).length > 0 && (
                    <div className="border-t border-green-900 pt-1 mt-1">
                      ...
                      <strong className="text-yellow-300 font-mono">
                        {(geometry.bendPoints ?? []).map(bp => `${(bp.angle ?? 0).toFixed(3)}Â°`).join(" | ")}
                      </strong>
                    </div>
                  )}
                </div>
      ...
      {geometry && (() => {
        ...
        // Step 1 risk logic (document): bend count â†’ thickness â†’ material
              ...
              <div className="mt-3 space-y-2">
                <div className="bg-zinc-800 rounded-lg border border-zinc-700 p-2.5 space-y-1.5 text-[10px]">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-yellow-300">{totalBendDeg.toFixed(1)}Â°</span>
                  </div>
                  ...
                  <div className="text-[9px] text-zinc-500 leading-tight">
                    Formula: bendsÃ—2{materialThickness <= 0.5 ? " +1 (thin sheet)" : ""}{materialType === "SS" ? " +2 (SS springback)" : ""}
                  </div>
                </div>
                <div className={`rounded-lg border p-2.5 text-[10px] ${
                  ...
                  <div className="text-[9px] text-zinc-500 mt-1 leading-tight">
                    {riskLevel === "low" && "Good formability â€” standard process applies"}
                    {riskLevel === "medium" && "Monitor closely â€” reduce forming speed if needed"}
                    {riskLevel === "high" && "High cracking risk â€” large radius, slow speed required"}
                  </div>
                </div>
                    ...
                    <div className="space-y-1.5">
                      {tightBends.length > 0 && (
                        <div className="text-[10px] bg-orange-950 border border-orange-800 rounded p-2 space-y-0.5">
                          <div className="font-bold text-orange-300">âš  Tight Radius Detected</div>
                          <div className="text-orange-400">{tightBends.length} bend(s) with R &lt; 2Ã—t â€” More passes needed</div>
                          <div className="text-zinc-500">Rule: Tight radius â†’ increase stations to reduce forming stress</div>
                        </div>
                      )}
                      {highAngleBends.length > 0 && (
                        <div className="text-[10px] bg-red-950 border border-red-800 rounded p-2 space-y-0.5">
                          <div className="font-bold text-red-300">â†‘ High Angle â€” Overbend Required</div>
                          <div className="text-red-400">{highAngleBends.length} bend(s) &gt; 90Â° â€” Springback over-form applied</div>
                ...
                {bendCount === 0 && (
                  <div className="text-[10px] text-amber-400 bg-amber-950 border border-amber-800 rounded p-2">
                    âš  No bend points detected. Upload a profile with bends for full analysis.
                  </div>
                )}
              </div>
            )}
            ...
            {stationSuggestion && (
              <div className="bg-blue-950 border border-blue-700 rounded-lg p-2.5">
                ...
                <div className="text-[10px] text-blue-200 space-y-0.5 mb-2">
                  ...
                  <div>Total bend angle: <strong className="font-mono">{stationSuggestion.totalBendDeg.toFixed(3)}Â°</strong></div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {numStations === stationSuggestion.recommended ? "âœ“ Applied" : "Apply"}
                  </button>
                </div>
              </div>
            )}
      ...
      {/* MACHINE SPECS */}
      <div className="p-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.045)" }}>
        <SectionHeader title="Machine Specs" icon={<span className="text-sm">ðŸ­</span>} expanded={sections.machine} onToggle={() => toggleSection("machine")} />
        {sections.machine && (
          <div className="mt-3 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                {autoShaft ? (
                  <div className="relative">
                    ...
                    <div className="text-[9px] text-green-500 mt-0.5">
                      Ã˜{rollDiameter}Ã—28% = {(rollDiameter*0.28).toFixed(0)}mm â†’ ISO Ã˜{shaftDiameter}
                    </div>
                  </div>
                ) : (
                  <input
            ...
            <div>
              ...
              <div className="flex gap-1">
                ...
                <button
                  title={`Auto clearance: thickness Ã— 0.1 = ${(materialThickness * 0.1).toFixed(3)} mm`}
              ...
              <div className="text-[10px] text-zinc-500 mt-0.5">
                Formula: tÃ—0.1 = {(materialThickness * 0.1).toFixed(3)} mm &nbsp;|&nbsp; Gap = tÃ—1.1
              </div>
            </div>
            <div className="bg-zinc-800 rounded p-2 border border-zinc-700 text-[10px] text-zinc-400 space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-green-300 font-bold">Ã˜{shaftDiameter} mm {autoShaft ? "(auto)" : "(manual)"}</span>
              </div>
              <div className="flex justify-between">
                <span>Keyway (DIN 6885-A):</span>
                <span className="text-yellow-300 font-medium">{keyway.width}Ã—{keyway.height} mm</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-zinc-300 font-medium">Ra 0.8 Âµm</span>
              </div>
              <div className="text-[9px] text-zinc-600 pt-1 border-t border-zinc-700">
                Full keyway + locknut + tolerance â†’ Generate Roll Tooling
              </div>
            </div>
          </div>
        )}
      ...
      <div className="p-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.045)" }}>
        <SectionHeader title="Machine Sizing Calculator" icon={<span className="text-sm">ðŸ§®</span>} expanded={sections.sizingCalc} onToggle={() => toggleSection("sizingCalc")} />
        {sections.sizingCalc && (
          <div className="mt-3">
      ...
      {/* POST PROCESSOR */}
      <div className="p-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.045)" }}>
        <SectionHeader title="Post Processor" icon={<span className="text-sm">ðŸ–¨</span>} expanded={sections.postproc} onToggle={() => toggleSection("postproc")} />
        {sections.postproc && (
          <div className="mt-3 space-y-2.5">
            ...
            {selectedPP && (
              <div className="bg-white/[0.02] border border-white/[0.06] rounded-lg p-2.5 text-[10px] text-zinc-500 space-y-2">
                <div className="text-zinc-300 font-semibold text-[11px]">{selectedPP.icon} {selectedPP.brand} â€” {selectedPP.name}</div>
                ...
                {selectedPP.id === "delta_2x" && (
                  <div className="mt-1 bg-amber-950/40 border border-amber-700/40 rounded p-2 space-y-1">
                    <div className="text-amber-400 font-bold text-[10px] mb-1">â¬› Delta 2X â€” 4 Machine Commands</div>
                    {[
                      { old: "M3",   newCmd: "M4",    desc: "Spindle direction â€” CCW rotation" },
      ...
      {/* G-CODE SETTINGS */}
      <div className="p-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.045)" }}>
        <SectionHeader title="G-Code Settings" icon={<span className="text-sm">ðŸ“„</span>} expanded={sections.gcode} onToggle={() => toggleSection("gcode")} />
        {sections.gcode && (
          <div className="mt-3 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <div>
      ...
      <div className="p-4 space-y-2.5">
        ...
        {/* Section model gate â€” must be selected at workflow start */}
        {sectionModelRequired && (
          <div className="flex items-start gap-2 rounded-lg px-3 py-2 text-[10px] text-violet-300 border border-violet-600/30 bg-violet-950/30 mb-1">
            <span className="text-violet-400 mt-0.5">âš </span>
        ...
        {/* DXF dimension confirmation gate â€” blocks generation until ALL dimensions confirmed */}
        ...
        {/* Validation gate â€” blocks ALL generation when validation has failed */}
        {validationFailed && (
        ...
        >
          ...
          1. Power Pattern
          <span className="ml-1 text-blue-300 font-mono text-[10px]">({materialType} Ã—{MATERIAL_DATABASE[materialType].springbackFactor})</span>
        </button>
        <button
      ...
      <div className="p-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.045)" }}>
        <SectionHeader title="Strip Width Calculator" icon={<span className="text-sm">ðŸ“</span>} expanded={sections.stripCalc} onToggle={() => toggleSection("stripCalc")} />
        {sections.stripCalc && (
          <div className="mt-3 space-y-2.5">
            ...
            <div>
              <label className="text-[10px] text-zinc-500 block mb-0.5">
                Inside Bend Radius (mm) â€” <span className="text-zinc-700">0 = auto (1.5Ã—t)</span>
              </label>
              <input
            ...
            >
              {stripLoading ? (
                ...
                <><span>ðŸ“</span> Calculate Strip Width</>
              )}
            </button>
            {stripCalcResult && (
              <div className="space-y-1.5">
                ...
                <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3">
                  ...
                  <div className="text-[10px] text-blue-600 mt-1">
                    Add 2â€“3 mm trim allowance â†’ Order {Math.ceil(stripCalcResult.totalFlatWidth + 3)} mm strip coil
                  </div>
                </div>
                {/* Breakdown */}
      ...
      {/* Status messages â€” channel-aware styled display */}
      {error && (() => {
        ...
        return (
          <div className={`mx-4 mb-4 p-3 rounded-lg text-xs border flex items-start gap-2 ${style.bgColor} ${style.borderColor} ${style.textColor}`}>
            <span className="flex-shrink-0 mt-0.5">
              {style.channel === "success" ? "âœ“" : style.channel === "warning" ? "âš " : style.channel === "error" ? "âœ•" : "â„¹"}
            </span>
            <span>{errStr.replace(/^[âœ“âš âœ•â„¹âœ…]\s*/, "")}</span>
          </div>
        );
      })()}
