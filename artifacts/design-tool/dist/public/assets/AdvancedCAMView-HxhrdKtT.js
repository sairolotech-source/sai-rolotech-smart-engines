...
(Vc=${u} m/min | Fz=${a}mm/t | Ap=${s.toFixed(1)}mm | Ae=${n.toFixed(1)}mm)
...
M30
%`}function _(t,p,r,m,d){const c=b[p],h=Math.round(1e3*c.Vc/(Math.PI*t)),u=Math.round(h*c.Fz*4),a=t*.5,o=t*.55,n=Math.ceil(m/a);return`%
...
T02
G43 H02 Z100.0
G00 X${(r/2).toFixed(2)} Y0 S${h} M03
M08
G00 Z2.0
G01 Z-${(t*.8).toFixed(1)} F${Math.round(u*.3)}
; --- TROCHOIDAL PASSES ---
${Array.from({length:Math.min(n,12)},(s,l)=>{const g=a*l;return`G03 X${(r/2+o).toFixed(2)} Y${g.toFixed(2)} I${o.toFixed(2)} J0 F${u}
G03 X${(r/2).toFixed(2)} Y${(g+a).toFixed(2)} I${(-o).toFixed(2)} J0 F${u}`}).join(`
`)}
G01 Z50.0 F3000
M05 M09
G28 U0. W0.
M30
%`}function O(t,p,r,m,d){const c=b[p],h=Math.round(1e3*c.Vc/(Math.PI*r)),u=c.Fz*5,a=c.Fz*2,o=2,n=d==="siemens840d"?"SIEMENS 840D":d==="haas"?"HAAS":d==="mazak"?"MAZAK":"FANUC";return`%
O0003 (G71 OD ROUGHING + G70 FINISH — ${b[p].name.toUpperCase()})
(CONTROLLER: ${n})
(DIA: Ø${r}mm | LENGTH: ${m}mm | TOOL: ${t}mm CCMT)
G21 G18 G40 G97 G99
T0101 (OD ROUGHING CNMG${t<20?"120408":"160608"})
G96 S${Math.round(c.Vc)} M03
M08
G00 X${(r+10).toFixed(1)} Z5.0
; --- G71 ROUGHING CYCLE ---
G71 U${o.toFixed(1)} R0.5
G71 P10 Q80 U0.3 W0.1 F${u.toFixed(3)}
; --- CONTOUR DEFINITION (N10 to N80) ---
N10 G00 X${(r*.4).toFixed(1)}
    G01 Z0 F${a.toFixed(3)}
    G01 X${(r*.5).toFixed(1)} Z-2.5
    G01 Z-${(m*.3).toFixed(1)}
    G02 X${(r*.65).toFixed(1)} Z-${(m*.35).toFixed(1)} R${(r*.075).toFixed(1)}
    G01 Z-${(m*.7).toFixed(1)}
    G01 X${(r*.85).toFixed(1)} Z-${(m*.85).toFixed(1)}
N80 G01 X${(r+5).toFixed(1)}
; --- G70 FINISH PASS ---
G70 P10 Q80 F${a.toFixed(3)} S${Math.round(c.Vc*1.3)}
G00 X${(r+30).toFixed(1)} Z50.0
M05 M09
; --- GROOVING (optional) ---
T0202 (GROOVE TOOL 3mm)
G97 S${Math.min(h,800)} M03
G00 X${(r+5).toFixed(1)} Z-${(m*.45).toFixed(1)}
G01 X${(r*.55).toFixed(1)} F0.04
G04 P300
G01 X${(r+5).toFixed(1)} F0.3
G00 X${(r+30).toFixed(1)} Z50.0
M05 M09
G28 U0. W0.
M30
%`}function J(t,p,r,m){const d=p-1.0825*t,c=(p-d)/2,h=m==="siemens840d"?`; SIEMENS 840D — USE CYCLE97 INSTEAD
...
(THREAD DEPTH: ${c.toFixed(3)}mm | LENGTH: ${r}mm)
${h}
G21 G18 G40 G97 G99
...
G76 X${d.toFixed(3)} Z-${r.toFixed(1)} P${Math.round(c*1e3)} Q${Math.round(c*.4*1e3)} F${t.toFixed(3)}
